...
- 保留泛型：
   - ```Class``` 无法保留***复杂的***泛型信息，如 ```List<T>/Map<K,V>``` 里的 ```<T>``` 和 ```<K,V>```会被擦除
   - 使用 ```TypeReference``` 的匿名实现类来保留泛型信息，但不能是 ```<T>```，会被擦除，也就是各个类型必须明确
   - 使用 ```JavaType``` 由内到外逐层创建完整的类来保留泛型信息，但不能是 ```<T>```，会被擦除，也就是各个类型必须明确
